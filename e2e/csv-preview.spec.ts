...
test.describe("CSV preview", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/?path=/home/csv-preview");
  ...
  test("renders CSV records as a virtualized table and keeps invalid CSV as text", async ({
    page,
  }) => {
    await page.locator('[data-path="/home/csv-preview/people.csv"]').click();
    ...
    await page.locator('[data-path="/home/csv-preview/broken.csv"]').click();
    ...
    await page.screenshot({
      path: "evidence/ac-4-markdown-preview-unchanged.png",
    });
  });

  test("bounds large CSV previews and virtualizes rows while scrolling", async ({
    page,
  }) => {
    await page
      .locator('[data-path="/home/csv-preview/many-people.csv"]')
      .click();
  ...
  test("keeps wide CSV columns aligned and horizontally reachable", async ({
    page,
  }) => {
    await page.locator('[data-path="/home/csv-preview/wide.csv"]').click();
    ...
    const alignment = await page
      .locator(".preview-csv")
      .evaluate((scroller) => {
        const header = scroller.querySelectorAll<HTMLElement>(
          '[role="columnheader"]',
        )[2];
