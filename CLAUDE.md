...
- Native qualification must bind resource sampling to the verified manifest binary, retain run-specific driver output on failures, and normalize Rust `Duration` units before calculating startup percentiles. On macOS, a signal exit leaves Node's `exitCode` null, so startup survival and cleanup must inspect `signalCode` too and force-kill a process that ignores graceful termination.
