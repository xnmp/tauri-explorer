...
| **PaneContainer** | `PaneContainer.svelte` | Hosts the active tab's content: a `PaneLayoutView` tree for explorer tabs, `GitGraphView` for graph tabs. The tab strip itself lives in `TitleBar` (#229) |
