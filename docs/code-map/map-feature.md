...
- Move ownership: `src-tauri/src/files/move_plan.rs` supplies both Linux recovery reservations and exact resolved source/target paths to `src-tauri/src/files/move_execution.rs`. Forward `file_mutation::move_entry` and the native history move adapter share that executor; physical refresh directories and cleanup warnings return to their existing outer owners. `src/lib/api/files.ts` consumes the session-fenced settled mutation reply. `src-tauri/test_support/file_move_recovery.rs` covers competing source/target reservations and alias binding. Durable moves (opt-in `durable-move-recovery`) journal that plan through `src-tauri/src/files/recovery/forward_move.rs` + `move_execution.rs` + `move_transition.rs`: user publication remains one no-replace rename after durable capability preflight, an overwrite retains its displaced original, and a cross-filesystem move parks its source only after publication. Undo consumes the durable record by identity. Cut/paste and drag-drop moves now go through the ordered move session below. ADR 0023 uses `src-tauri/src/files/recovery/move_retention.rs` and `src-tauri/src/files/recovery/move_retirement.rs` to retain completed Undo until explicit discard and reclaim verified restored redundancy through independent root checkpoints; `src-tauri/src/files/recovery/move_cleanup.rs` records exact descendant authority before directory removal so resumption cannot sweep foreign children. Native filesystem and process-kill contracts live in `src-tauri/test_support/recovery_move_retirement.rs`; `src-tauri/src/files/recovery/move_capability_model.rs` and `src-tauri/src/files/recovery/move_capability.rs` journal a real per-volume rename probe before move roots, with interrupted-probe cleanup and legacy-safe versioning; contracts in `src-tauri/test_support/recovery_move_capability.rs`. Native UI acceptance exercises real file/directory moves in `e2e-tauri/specs/move-retirement.spec.ts` (#736).
- Ordered copy sessions: `src/lib/state/copy-operations.ts` is lazily loaded by paste/drop copy paths and calls `src/lib/api/copy-session.ts` with `src/lib/domain/copy-session.ts` contracts. `src-tauri/src/files/copy_session.rs` + `src-tauri/src/files/copy_session/model.rs`, `src-tauri/src/files/copy_session/control.rs`, `src-tauri/src/files/copy_session/worker.rs` own a bounded ordered request, live conflict inspection, item/non-reused-nonce/renderer-fenced replies, one cancellation scope and external effect receipts. `file_mutation::copy_entries` settles one mixed ordinary/replacement inverse. Progress travels on the request channel with item identity; prompts occupy no blocking worker; Linux inspected versions bind anchored source reads, replacement admission and physical publication. Contracts: `src-tauri/test_support/copy_session.rs`, `src-tauri/test_support/file_history_publication.rs`, `tests/api/copy-session.test.ts`, `tests/state/copy-operations.test.ts`.
- Ordered move sessions reuse that engine: `src-tauri/src/files/move_session.rs` is the second `Work` implementation, so ordering, conflict pauses, cancellation, the bounded diagnostic budget and the completed prefix cannot drift from the copy contract. It rejects a directory relocated into its own subtree before prompting, treats a same-directory paste as a success that touches nothing, and routes the effect to the durable journal under `durable-move-recovery` or to `file_ops::move_entry_impl` otherwise. `file_mutation::move_entries` reserves the destination plus every source parent and settles one inverse per committed item — the durable record when there is one, an `Action::Move` otherwise, never both. A receipt whose source removal did not finish becomes `MutationUncertain`, not a success, so it mints no inverse and keeps the cut clipboard; a same-directory or aliased no-op is marked `unchanged` and produces no forward history entry at all. `src/lib/state/session-operations.ts` is the one renderer presentation for both sessions, specialised by `copy-operations.ts` and `move-operations.ts` for `paste-operations.ts` (cut) and `drop-operations.ts`; the renderer records no inverse of its own, deduplicates repeated move sources, and releases a cut clipboard only when every requested item arrived. Plugin-driven moves still use `file-transfer.ts`. Contracts: `src-tauri/test_support/move_session.rs`, `tests/state/move-operations.test.ts`, `tests/state/drop-operations.test.ts`, `tests/state/paste-progress.test.ts`, `e2e-tauri/specs/file-move-recovery.spec.ts`.
- Production copy: `src-tauri/src/file_mutation.rs` owns acknowledged copy command admission and history settlement. `src-tauri/src/files/file_ops.rs` keeps shared naming/progress and an owned task registration; Linux replacements use `src-tauri/src/files/recovery/forward_copy.rs` through the runtime's owned publication boundary. `src-tauri/test_support/recovery_forward_copy.rs` and `src-tauri/test_support/recovery_runtime.rs` cover actual effects, changed targets, aliases, waiter loss, panic and inventory failure. Legacy singleton receipts prevent ordinary Copy inverse recording for replacements in `src/lib/state/file-transfer.ts`. Paste/drop copies now use the lazy native session and its grouped ordinary/replacement history; move history remains renderer-owned. Overwrite Undo/Redo uses exact native replacement tokens; artifact retirement is implemented per ADR 0023 (see the retention entry below).
...
- Native recovery acceptance: `src-tauri/test_support/file_recovery_native.rs` seeds real durable replacements and observes actual Channel destruction with native registration IDs. `src/test-support/file-recovery-probe.ts` exposes tokened raw IPC leases; `e2e-tauri/specs/file-recovery.spec.ts` verifies restored filesystem payloads, renderer reload/destruction, stale-session rejection and delivery of fresh durable generations. `e2e-tauri/specs/move-retirement.spec.ts` exercises production cross-filesystem file/directory moves, dialog discard of both retained roots, exact accounting changes and preservation after external destination edits. `e2e-tauri/specs/cross-device-move-history.spec.ts` moves files and trees between /dev/shm and $HOME and undoes them through native history: the default build cycles Undo/Redo, and `durable-move-recovery` (declared by `TAURI_E2E_DURABLE_MOVE_RECOVERY=1`) journals the move and offers no Redo. `.github/workflows/native-recovery.yml` builds the opt-in recovery binary and runs these suites through `gatedDescribe` (`e2e-tauri/gated-suites.ts`), `file-recovery.spec.ts` in its own step because its launch fixture must not be reseeded (#774).
...
- `src-tauri/src/files/permanent_delete.rs`, `src-tauri/src/files/permanent_delete/tree.rs`, `src-tauri/test_support/permanent_delete.rs` — identity-bound permanent deletion via private sibling staging, restore-on-substitution and phase-classified residue; injectable `Operations` seam drives final-seam substitution and partial-removal tests (#739).
...
- `src-tauri/src/files/entry_plan.rs`, `src-tauri/src/files/entry_execution.rs`, `src-tauri/src/files/file_ops.rs` — one owned plan supplies simple-entry command targets, refresh parents and rename history names; the filesystem worker rechecks collisions and consumes that exact request. Linux commands lazily admit through `src-tauri/src/files/recovery/runtime.rs`, execute captured paths and retain traversed parent-alias reads through publication. Native rename inverses share this executor, return cleanup warnings/physical refresh parents, and bind the next direction to the physical parent. Incoherent source/target parent captures fail before effects. `src-tauri/test_support/file_entry_execution.rs` and `src-tauri/test_support/file_history_rename_admission.rs` cover worker lifetime and actual inverse exclusion/cycles. Other platforms and operation roots remain pending.
...
- `src-tauri/src/file_history/plan.rs` — fixes inverse direction, paths and exact restore receipts before dispatch; invalid leaves remain in execution order, and panic refresh uses planned parents. Move execution and refresh derive the original parent from `source_path`, ignoring redundant legacy metadata. Linux trash/restore adapters take prepared recovery admission; other inverse families retain their own coverage boundaries.
...
- `src-tauri/src/files/batch/mod.rs`, `files/batch/model.rs` — bounded, stable selection admission and worker-independent progress; pooled and dedicated workers share read-only setup/execution/cleanup handling, confirmed siblings survive a panic, uncertain work stops later attempts. Linux forward trash prepares a complete selection under the recovery coordinator revision fence, then the batch worker holds admission through capture cleanup; inverse callers retain the original pooled setup path.
...
- `src-tauri/src/files/freedesktop_trash/plan.rs` — prepare exact source, payload, final/staged metadata and directory actions without mutation; mounted trash predeclares its personal fallback. Execution preserves those names and verifies object/mount identity, while safe first-use directory sharing does not introduce unplanned permission repair. Whole-selection forward admission includes these exact namespaces; durable artifact promotion remains open.
- `src-tauri/src/files/freedesktop_trash/selection.rs` — observes every selected source/alias before destination planning; shares exact layouts and bounds/indexes all candidates before running aligned item plans. `src-tauri/src/files/recovery/resources.rs` supplies the shared role-aware namespace policy. Linux forward deletion reserves the captured claims through `Runtime::admit_prepared`; requested receipt keys remain separate from physical execution bindings. Native trash inverses use the same prepared admission; durable trash promotion remains open.
- `src-tauri/src/files/freedesktop_trash/restoration.rs` — prepare exact target/payload/metadata and missing-parent claims before native restoration; reuse only verified batch-created parents, keep per-item results and conservative refresh effects in the external batch ledger. `src-tauri/test_support/trash_restoration_admission.rs` and `src-tauri/test_support/file_history_trash_admission.rs` cover overlap, substitutions, partial creation, hardlinks, aliases, native-path display collisions and actual inverse adapters.
...
Planned move journal authority: `src-tauri/src/files/recovery/move_model.rs` owns the immutable resource/volume/artifact contract. `src-tauri/src/files/recovery/coordinator.rs` preserves planned records on reopen and rejects them from copy history execution. Durable move execution (#685): `src-tauri/src/files/recovery/move_transition.rs` is the pure legal-transition function encoding the crash ordering (publish before park, park before source removal, no restoration after removal); `src-tauri/src/files/recovery/move_execution.rs` is the concrete executor (stage, displace, publish, park, remove, restore) using private artifact roots; `src-tauri/src/files/recovery/forward_move.rs` is the production orchestration (admission, `MoveSpec` binding, promotion, execution) producing `FileMutationReceipt`/`MoveRecoveryReceipt`. Their contracts live in `src-tauri/test_support/recovery_move_transition.rs`, `src-tauri/test_support/recovery_move_execution.rs` and `src-tauri/test_support/recovery_forward_move.rs`.
