...
> **Not for locating change sites.** To find the files a feature, bug, or perf task touches, use [docs/code-map/](code-map/) — `map-feature.md` for cross-layer work, `map-playbook.md` for task-shaped changes. A controlled study ([code-map/STUDY.md](code-map/STUDY.md)) measured this document and `architecture/` as *net-negative* for that purpose versus reading no docs at all: +4.6% search cost, −4.8% answer quality, 2.5× more wrong files. Read on for design and orientation; search elsewhere.
>
