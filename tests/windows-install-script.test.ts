  ...
  () => {
    ...
    it("builds existing and cloned checkouts, preserves MSI paths with spaces, and cleans clones", async () => {
      ...
      try {
        ...
        expect(result.existingBuilds).toEqual([
          "bun install",
          "bunx tauri build",
        ]);
        expect(result.temporaryBuilds).toEqual([
          "bun install",
          "bunx tauri build",
        ]);
        expect(result.gitCommands).toEqual([
          [
            "clone",
            "--depth",
            "1",
            "https://github.com/xnmp/tauri-explorer.git",
            result.temporaryCheckout,
          ],
        ]);
