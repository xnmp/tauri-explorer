...
| Git commit graph (#51/#57/#58/#179) | `git_log.rs` (git_log/git_refs/git_commit_files via libgit2; weaves stashes in as pseudo-commits), `api/git-log.ts`, `domain/git-graph.ts` (`assignLayout` vertex/branch model: continuous first-parent-chain polylines, branch-owned colors with reuse, merge-edge snapping — behavioral parity with VSCode Git Graph, reimplemented not ported), `GitGraphView.svelte` (one SVG path per branch in a shared underlay so lines never break at row boundaries; synthetic “Uncommitted Changes” row, stash ring markers, combined local+remote ref chips), toggled per-pane via the `git.showGraph` palette command (#272): the active pane swaps between its file listing and the repo's graph (`TabPane.gitGraph`, persisted per pane; pre-#272 git-graph TABS migrate to single-pane explorer tabs on load). Inline details (#221): clicking a row (incl. the synthetic Uncommitted Changes row) expands meta + files below it — the SVG stretches via `RowExpand`; clicking a file shows its diff (`git_commit_file_diff` for commits, `git_diff` for the working tree). Default shortcut Ctrl+Alt+G |
| SCM panel alongside graph + per pane (#333/#334) | `ScmPanel` renders in BOTH branches of `ExplorerPane` (explorer and git-graph), so working-tree status stays visible while the graph is open. Per-pane stores via `getScmStore(paneId)` in `scm.svelte.ts` — each panel follows its own pane's explorer, two panes on different repos show independent panels; shared module-level summary cache + `warmScmSummary`; panels `release()` their store on unmount; repo watchers refcounted in `git.rs` so panes/windows sharing a repo can't drop each other's watcher. Window-global surfaces (preview diff) follow `windowTabsManager.activePaneId` |
| Git graph columns + branch filter (#341/#342) | `GitGraphView.svelte` header (Message/Author/Date/Commit) with drag handles: author/date via `usePersistedPanelWidth` (new `invert` option — handle on the column's left edge), the lane gutter is lane-derived until first dragged then a fixed persisted width that clips overflow. Branch filter: header button → popover (text filter, checkboxes, hover "only", "All branches"); selection persists per repo and feeds `GitLogOptions.branches` — `git_log.rs` seeds the revwalk from only those tips (mock mirrors with a parents-BFS). Filtered pages bypass the warm snapshot cache |
