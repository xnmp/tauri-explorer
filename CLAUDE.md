...
- Watcher callbacks are delayed work: capture the explorer path when queuing a refresh and drop the callback if that explorer navigates before it runs. Otherwise an event attributed to the old directory refreshes the new one and corrupts both directories' cadence.
