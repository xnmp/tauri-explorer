...
describe("getBookmarkDropHint", () => {
  ...
  it("uses persisted native drag data to advertise pinning in a destination window", () => {
    const kind = getEffectiveDragKind(null, { kind: "directory" });

    expect(getBookmarkDropHint(kind, true)).toBe("Drop to pin");
  });
});
