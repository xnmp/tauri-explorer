...
import { draggedResizeSize, clampResizeSize, resizeSizeFromKey } from "$lib/domain/resize-size";
...
describe("panel width contracts", () => {
  it("normalizes malformed saved values and clamps finite extremes", () => {
    for (const value of [null, undefined, "300", NaN, Infinity, {}, []]) expect(clampResizeSize(value, options)).toBe(240);
    expect(clampResizeSize(-1e100, options)).toBe(180);
    expect(clampResizeSize(1e100, options)).toBe(400);
  });
  it("converts visual movement to CSS width and honors left-side handles", () => {
    expect(draggedResizeSize(240, 60, 1.5, options)).toBe(280);
    expect(draggedResizeSize(240, 60, 0.8, options)).toBe(315);
    expect(draggedResizeSize(240, -60, 1.5, { ...options, invert: true })).toBe(280);
    for (const scale of [0, -1, NaN, Infinity]) expect(draggedResizeSize(240, 60, scale, options)).toBe(240);
    expect(resizeSizeFromKey(240, "ArrowRight", { ...options, invert: true })).toBe(230);
  });
  it("coalesces movement, publishes the last release position and persists once", () => {
    ...
    expect(owner.value).toBe(240); expect(frames).toHaveLength(1);
    owner.finish();
    expect(owner.value).toBe(280); expect(persisted).toEqual([280]);
  ...
  it("cancellation discards queued movement but retains and saves rendered work", () => {
    ...
    expect(owner.value).toBe(260); expect(persisted).toEqual([260]);
  ...
  it("keyboard sizing respects bounds and retires pending pointer work", () => {
    ...
    expect(owner.value).toBe(250);
    owner.key("Home"); expect(owner.value).toBe(180);
    owner.key("End"); owner.key("ArrowRight"); expect(owner.value).toBe(400);
...
it("automatic width follows its source until an effective manual adjustment", () => {
  ...
  expect(owner.value).toBe(200);
  automatic = 300; expect(owner.value).toBe(300); expect(persisted).toEqual([]);
  owner.start(100, 1); automatic = 350;
  expect(owner.value).toBe(300); // active gesture captures its displayed origin
  owner.finish(); expect(owner.value).toBe(350); expect(persisted).toEqual([]);
  owner.start(100, 1); owner.move(100); owner.finish();
  automatic = 320; expect(owner.value).toBe(320); expect(persisted).toEqual([]);
  owner.start(100, 1); owner.move(130); owner.finish();
  automatic = 200; expect(owner.value).toBe(350); expect(persisted).toEqual([350]);
});
...
it("automatic sizing is bounded, keyboard adjustment becomes explicit, and stored preference wins", () => {
  ...
  expect(automatic.value).toBe(400);
  automatic.key("ArrowLeft"); expect(automatic.value).toBe(390);
  const stored = createPanelResize(260, options, deps);
  expect(stored.value).toBe(260);
});
...
it("returning a manual drag to its automatic origin persists the explicit preference", () => {
  ...
  source = 300; expect(owner.value).toBe(240); expect(saved).toEqual([240]);
});
