...
- **Floating, over-wide overlays anchored to a grid cell clip at the container edge.** The Tiles rename box was `position:absolute; left:50%; transform:translateX(-50%); width:max(180px, 100%+48px)` so it spilled past its tile — fine mid-grid, but the leftmost/rightmost tile's box got cut off by the pane boundary, and short names looked oversized. Fix: make it in-flow (`width:100%`), centered text, and auto-grow *height* to fit wrapped content. Long names wrap and stay fully visible; nothing overflows the pane. Lesson: prefer in-flow + wrap over a floating box that overflows neighbours when the cell can sit against a hard edge.
