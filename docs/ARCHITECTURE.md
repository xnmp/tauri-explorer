...
| Tests | Vitest (unit), Playwright (e2e: Chromium + WebKit proxy for the macOS webview — `WEBKIT=1 npx playwright test --project=webkit`), WebdriverIO real-binary smoke (Linux/Windows), macOS launch smoke (`.github/workflows/macos-smoke.yml`: boots the built app, asserts startup) |
