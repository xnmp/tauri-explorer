...
| Extended native soak (opt-in)         | Build the E2E-enabled Tauri binary above, then `SOAK_DURATION_MS=14400000 SOAK_SEED=<repro-seed> SOAK_EXPECTED_DISPLAY_SCALE=<native-scale> bun run test:e2e:tauri:soak` (`SOAK_MAX_CYCLES=1` gives a bounded real-native harness check; reports and failure screenshots land in `qualification-results/`)                                                                                                                       |
