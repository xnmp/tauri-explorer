...
- `domain/git-network-operation.ts`, `state/git-graph-refresh.ts` — F5 refresh bus plus importable `createReloader` concurrency state machine, local-change filter, real-commit paging counter, and shared network-operation phase state: GitGraphView registers its fetch+reload per pane; `gitGraph.refresh` dispatches to the active graph pane, while the Rust task phase removes pull cancellation once protected local fast-forward begins (#432, #444, #528)
