...
- `state/git-graph-cache.ts` — bounded per-repo graph snapshot cache + `warmGraphSnapshot`/`fetchPage0Snapshot` (moved out of GitGraphView so `git-warm.ts` imports state, not a component); retains the supported 12-tab graph fan-out for remounts, and evicts a repo's snapshots on external (watcher) git changes so a remount never paints stale history (#433, #505, arch Finding 7)
