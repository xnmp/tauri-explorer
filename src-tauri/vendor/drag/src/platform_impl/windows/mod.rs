...
// LOCAL PATCH: thread-local OLE init. OleInitialize is per-thread (STA),
// so the original process-wide `Once` only initialized OLE on whichever
// thread first called `start_drag`. We currently dispatch from the main
// thread only, but the per-thread variant is correct under any caller.
thread_local! {
...
fn init_ole() {
    OLE_INITIALIZED.with(|cell| {
        ...
        // S_FALSE is returned when the thread is already STA-initialized;
        // both that and S_OK are acceptable. Errors here (e.g.
        // RPC_E_CHANGED_MODE) will surface downstream when DoDragDrop /
        // SHCreate* fails.
        unsafe {
        ...
        match item {
            DragItem::Files(files) => {
                ...
                let data_object: IDataObject = match get_file_data_object(&paths) {
                    Some(obj) => obj,
                    None => return Err(crate::Error::UnsupportedWindowHandle),
                };
