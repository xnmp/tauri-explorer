...
describe("New folder tile sizing", () => {
  it("all thumbnail sizes have displaySize >= 48 for the folder SVG to render", () => {
    for (const [size, config] of Object.entries(THUMBNAIL_SIZE_CONFIG)) {
      expect(config.displaySize, `${size} displaySize`).toBeGreaterThanOrEqual(48);
    }
  });
  ...
  it("icon scale factor is displaySize / 64", () => {
    for (const [size, config] of Object.entries(THUMBNAIL_SIZE_CONFIG)) {
      ...
      expect(expectedScale, `${size} scale`).toBeGreaterThanOrEqual(0.75);
    }
  });
});
