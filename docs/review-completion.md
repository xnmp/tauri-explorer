...
## Architecture integration on dev — 2026-09-26

The follow-up PRs from the 2026-09-25 handover
([`docs/handoffs/architecture-review-2026-09-25.md`](handoffs/architecture-review-2026-09-25.md))
all merged into dev by squash, in dependency order:
...
| --- | --- | --- | --- |
| #752 → #745 | `d07731e4` | Native fixtures belong to the run cleanup root and are removed only after native process exit (`afterSession`/`onComplete`); a split launcher/worker lifecycle contract covers it | Remaining specs that `rmSync` fixtures while the app lives are #761 |
...
- #703/#709 — WebKitGTK renderer loss, and terminal keystrokes arriving out of
  order.
...
acceptance. The rows below keep their historical wording, with 2026-09-26
notes appended where that integration changed the state. Full checkpoint
...
| 1. Startup performance | Release Mac half-bounce recording, first presented frame and successful input, >=30 samples/scenario with p50/p95; cold, warm-cache, warm-window and restored optional surfaces; actionable profile-driven improvements | Instrumentation and payload budgets implemented; actual Mac measurements outstanding. 2026-09-26: #738, #747 and #753 landed Arch phase attribution, complete snapshot delivery, immutable listings and compact IPC. The Mac gate stays open in #696. |
