...
- `git-graph-cache.ts` — bounded per-repo git-graph snapshot cache + `warmGraphSnapshot`/`fetchPage0Snapshot`; extracted from GitGraphView so `git-warm.ts` no longer imports a component; retains the supported 12-tab fan-out and evicts a repo's snapshots on external (watcher) git changes so a remount never paints stale history (#433, #505, arch Finding 7).
