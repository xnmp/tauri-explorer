...
This delivery covers forward deletion commands on Linux. Direct inverse
`run_batch`, `trash_publication` and `restore_entries` callers retain their prior
ownership behavior and still need shared-admission integration ([#740](https://github.com/xnmp/tauri-explorer/issues/740)). Windows/macOS
also retain their existing platform implementations. It does not make trash a
durable recovery record or claim complete mutation-family coverage.
...
and (2) only, exactly as every other family does. Inverse deletion/restoration, ordinary copy outside the session and
