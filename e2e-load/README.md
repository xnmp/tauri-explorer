...
| `churn-leak` | 25× open/close tab cycles + 25× graph toggle in one pane (`LOAD_CYCLES=N` to scale) | Heap not returning to ~baseline after forced GC (leaked watchers/timers/snapshots) |
| `large-graph` | Single 5000-commit graph | Slow initial render, unbounded DOM rows (virtualization broken), slow deep-commit selection |
...
| `constrained-memory` | 8 tabs × 1000 commits under a 256MB JS heap cap | Renderer crash or unresponsive tabs (survival test, no timing assertions) |
