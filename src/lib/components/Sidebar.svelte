...
<script lang="ts">
  ...
  onMount(() => {
    ...
    // Capture phase so we read `dragState.current` BEFORE any drag source's
    // bubble-phase `ondragend` clears it. This removes the need for sources
    // to defer their clear via setTimeout — the HTML spec guarantees capture
    // fires before bubble.
