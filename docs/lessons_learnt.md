...
- **Adding a version stamp to `DEFAULT_SETTINGS` silently arms every code path that writes the settings object.** `init()`'s fallback branch promotes the localStorage cache into `settings.json` when the config file is missing or unreadable; because the stamp now rides along in the defaults, that write marked a blob that had never been through the ledger as already migrated — disarming this and every future migration for that install, permanently. The fix is to promote it **unstamped** (`delete promoted[SETTINGS_VERSION_KEY]` before the write) so the next launch migrates it normally. Whenever you add a "has been processed" marker to a defaulted object, audit every writer of that object, not just the one you were editing — the marker's *absence* is the signal, and defaults destroy absence.
- That fallback is unreachable on an unmodified upgrade lifecycle (`settings.json` persistence landed 2026-03-11 in `b64299cb`, three months before `tabTitleGitRoot` existed at all in `22a0e320` 2026-06-24, and `saveSettings` writes both stores in one call — so no install can hold that key without also holding a config file). It is reachable when the config file is lost or unreadable while localStorage survives: a user deleting the config to "reset", a permission-blocked write, a warm-window boot reading mid-truncate. **"Unreachable on the happy path" is not "unreachable"** — and the one-line unstamped fix costs nothing, whereas migrating the promoted blob outright would have flipped the live value and broken the existing `e2e/tab-title-git-root.spec.ts` fixture.
