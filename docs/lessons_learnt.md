...
## 2026-07-27 fix/530 git-graph branch-line jump: "same branch line" has no single source of truth
- **`assignLayout` decides which line a commit belongs to by CLAIM ORDER, not by topology** — the chain rooted at the topmost row that reaches a commit claims it, and later first-parent children join as merge-style edges instead. So "the commit above me on my line" is not derivable from `{oid, parents}` alone. Following `parents[0]` DOWN always matches a drawn edge and is unambiguous; going UP is genuinely ambiguous whenever two commits share a first parent (two branches rooted at the same commit). `stepOnBranchLine` takes the NEAREST qualifying commit above and says so in its docstring — reusing `vertices[].colorIndex` would be wrong (colors are recycled), and a per-vertex line id would be worse (a child whose line joins an already-drawn parent would jump nowhere despite a visible edge).
- The round-trip invariant that follows is easy to get backwards: with nearest-above winning, `newer(older(row))` lands in `[row, older)` — it can sit BELOW where you started, never above it. The fuzz test asserted `<= row` and caught the mistake immediately; a hand-picked example would not have.
- **A `when`-guarded shortcut still steals the key from the embedded terminal.** The terminal's ownership gate (`isShellReservedKey`, fed by `matchesAnyBinding`) deliberately skipped availability guards, so binding graph-only Ctrl+Up/Down made them dead keys at every shell prompt whose pane wasn't a graph: xterm declined the key because "the app owns it", then `findMatchingCommand` ran the guard, found nothing available, and dropped it. `matchesAnyBinding` now takes the same `isAvailable` predicate `findMatchingCommand` uses, and both gate callers pass it. Any new guarded binding on a combo readline uses (Ctrl+Arrow, Ctrl+A/E/K/W…) has this failure mode.
- The graph had **no arrow-key commit navigation at all** before this, so the shortcut is the first keyboard mover of `selected`. Two consequences: the jump target may be outside the virtualized render window (`.graph-scroller` needed a `bind:this` — it had no element ref), and selecting reflows every row below via `rowExpand`, so scroll-into-view must `await tick()` after `selectCommit` before reading `rowY`.
...
## test/high-load-stress-tests (#472): mock-FS fixtures are shared test state; quantized heap metrics can't detect leaks
- Adding entries to the shared mock filesystem (16 `load-repo-<i>` dirs under Documents for the e2e-load suite) is a change to EVERY e2e spec's baseline, not just the new suite's. It broke 132 tests — but only in list/tiles view modes, because the view-switch `beforeEach` right-clicks "empty space" in the file list to reach the view context menu, and with 16 extra folders the viewport had no empty space; the right-click hit a folder and opened the entry menu instead. Details mode passed (virtualized rows leave a gap), so a details-only regression run looked green. Fixes/lessons: (1) fixture additions must be opt-in — load-repos exist only when the page carries `?mockGitCommits=N`; (2) a regression check for a mock-data change must run ALL view modes, not the fast default.
- Chromium leak assertions need two flags and multi-pass GC, or they're tautologies: without `--enable-precise-memory-info`, `performance.memory.usedJSHeapSize` is quantized/rate-limited and heap deltas read exactly 0 regardless of behavior (the first suite run "passed" with +0.0MB — structurally unable to fail). With precise readings, a single `gc()` pass still left multi-MB of collectable garbage and false-positived a "leak" (+12.3MB) that vanished under 3× gc-with-settle (+1.6MB at DOUBLE the cycles). Leak verdicts need: precise flag + repeated GC passes + a scaling check (`LOAD_CYCLES=2N` — real leaks grow linearly, noise doesn't).
- Piping playwright through `| tail` masks its exit code (pipe exits with tail's 0), and rtk's summary of a playwright JSON report can invent "+N more failures" from passing-run JSON. For gating runs: redirect to a file, `echo EXIT=$?`, and count `"status": "unexpected"` in the report rather than trusting a filtered one-liner.

## fix/all-view-modes-e2e-helper (#474): a geometry-based "empty space" click is a time bomb; ALL_VIEW_MODES rot went unnoticed
- `switchViewMode` (e2e/helpers.ts) right-clicked `.file-list .content` at fixed `(x=10, y=height/2)` assuming empty space — and two specs carried private COPIES of the same helper (thumbnail-size, folder-thumbnails), so fixing the shared one still left 5 red. Every mock-listing growth silently moved the last row closer to the midpoint; once `/home/user` reached ~10 entries the click landed ON a row in details view, opened the ENTRY context menu (no View options), and every list/tiles variant died in `beforeEach` — 173 tests. Fix: derive the click point from the last `.entry-item`'s bounding box (+clamp for menu headroom) and delete the private copies in favor of the shared helper. Grep for copy-pasted helpers before declaring a helper fix complete.
- The rot pre-dated the day's four merges (bisected: broken at 9af17785 and earlier) — CI and the fast local default run details-only, so ALL_VIEW_MODES decayed invisibly. With several unrelated merges landing the same day, the 173-failure report looked like a fresh regression from any of them; a three-point A/B in a scratch worktree (base, mid, tip — ~3 min of test time) exonerated all of them before anyone chased ghosts in the day's diffs.

## fix/504-tab-titles-single-tab (#504): an extracted rule dropped a clause, and only the clause's own test would have caught it
- The exact bug was fixed once before, in `5ca49d76`, as an inline gate in `WindowTabBar.svelte`: `tabs.length > 1 || settingsStore.showWindowControls`. Then #140 (per-pane tabs) extracted the visibility rules into `domain/titlebar.ts` as `showWindowTabBar(tabCount, activeTabRenameable)` and **silently dropped the `showWindowControls` clause** — the extraction transcribed the shape of the rule, not all of it. The old fix had no test, so nothing went red. When you extract a component-local condition into `domain/`, diff the *clauses*, not the resulting expression, and bring a test per clause with it; an untested inline fix is one refactor away from being deleted.
- The regression is invisible in the title bar's own visibility: `showTitleBar` ORs in `showWindowControls`, so the row still rendered — just empty except for the drag region and the min/max/close buttons. A bug that only removes content *inside* a container that still renders will not trip any "is the chrome there?" assertion; assert on the content (`.tab-title` text), not the container.
- Note the two callers: `TitleBar.svelte` derives `tabStripVisible` to decide whether the row renders, and `WindowTabBar.svelte` derives `showTabArea` to decide whether the strip renders. They duplicate the same visibility call, so any new argument must be threaded through **both** or the row and its contents disagree.
- The fix landed as a NEW composed rule, `showTabStrip(tabCount, activeTabRenameable, showWindowControls)` = `showWindowTabBar(tabCount, activeTabRenameable) || showWindowControls`, rather than a third parameter on `showWindowTabBar`. Two reasons: the harness PR gate rejects any diff that edits an EXISTING test file (widening the signature forces every existing call site in `tests/domain/titlebar.test.ts` to change), and it keeps the strip's intrinsic reason to render separable from the row's. Make the new parameter REQUIRED, never defaulted — a defaulted `showWindowControls = false` would silently reproduce this exact bug at any call site that forgets it.


## feat/494 preview-info toggle: settings-dialog toggles are 0×0 checkboxes — e2e must click the slider
- `SettingsDialog.svelte`'s `.toggle input` is styled `opacity: 0; width: 0; height: 0` (the visible control is the sibling `.toggle-slider` span). Playwright therefore treats the checkbox as **hidden**: `expect(input).toBeVisible()` and `input.click()` both fail with "element is not visible", even though the row is plainly on screen. Drive it via `.setting-row:has([data-testid=…]) .toggle-slider` for the click and read state off the input with `toBeChecked()` (which works on hidden inputs). Any new settings e2e should copy that shape rather than rediscovering it.
- Adding a boolean setting is genuinely five touch points and no Rust: `Settings` field + `DEFAULT_SETTINGS` + getter/toggler in `settings.svelte.ts`, a `TOGGLE_SETTINGS` entry (that alone registers the palette command — `generateToggleCommands()` builds `toggleState`/`handler` for you), and a `rows.*` entry that must ALSO be pushed into the section's `*Rows` array in `SettingsDialog.svelte` or the whole section disappears when the search filter runs. The settings blob is serialized wholesale to `settings.json`, so persistence is free.
- Screenshot-as-assertion-byproduct works well here: capturing inside the asserting spec (`e2e/preview-auxiliary-info.spec.ts`) guarantees the image shows the state the test proved. Two gotchas — wait for `.preview-code` before the "info shown" shot or the preview body is still blank and the before/after pair differs by more than the feature; and `scrollIntoViewIfNeeded()` the settings row first, or it lands clipped at the dialog's bottom edge.
- Do NOT point `page.screenshot({path})` straight at a committed `evidence/` file: PNGs aren't byte-reproducible, so every ordinary `bun run test:e2e` dirties the tree, and a run that fails midway leaves a half-updated evidence set. Write to gitignored `test-results/` by default and gate the committed path behind `CAPTURE_EVIDENCE=1`.

## fix/481 debounce-tab-state-persistence: `localStorage.setItem` is a UI-thread fsync under WebKitGTK, and the frontend had no equivalent of `writeConfigQueued`
- **`localStorage` is not a cheap in-memory map in the real app.** Under WebKitGTK it is SQLite-backed and flushed synchronously in the UI process, so on a DRAM-less SSD (~70ms/flush measured for #472's `fsync-safe.sh`) one `setItem` can stall the frame. `window-tabs.svelte.ts` called `saveState()` from ~19 sites — every tab open/close/switch/rename/split/pane-focus — so *every tab interaction* carried a potential flush stall at the exact moment of the key press. This is why the app lagged harder than idle apps during a system-wide fsync storm. Treat any `savePersisted` on an interaction path as a disk write, not a variable assignment.
- **Config files already had a latest-wins serialized writer (`writeConfigQueued` in `state/persisted.ts`); localStorage had nothing.** The parallel now exists as `createCoalescedPersister(key, delayMs)` in the same module — a trailing window that holds the value and writes once the caller pauses. Reach for it for any key rewritten per interaction; don't hand-roll a `setTimeout` in the calling store.
- **Debouncing a persistence call means auditing its callers for who wanted durability.** `window-tabs` exports `save` for `use-window-lifecycle.ts`'s `beforeunload` handler and 30s interval save — callers whose entire purpose is "the state is stored when I return". Debouncing those inverts their meaning, so `save` maps to `writeNow()` (immediate) while the 19 internal sites map to `schedule()`. Same reason the persister flushes itself on `visibilitychange`→hidden / `pagehide` / `beforeunload` and on `dispose()`: a trailing timer that can be outlived by the page is a data-loss bug, and a *trailing* window (restarted on every schedule, so it fires when the user pauses) can in principle be starved forever by sustained input — the interval save is what bounds that.
- **The honest test seam is the write count for the key, not the store's getters.** `vi.spyOn(localStorage, "setItem")` filtered to `explorer-tabs`, plus a real-browser count with `Storage.prototype.setItem` patched in a Playwright `addInitScript`. Measured over 5×Ctrl+T + 5×Ctrl+Tab: **10 writes on the interaction path before, 0 after** (one trailing write lands once the burst ends), same 6 tabs restored either way. Assert *both* halves — a "0 writes" test alone is equally satisfied by a fix that just drops the state.
- **Left the theme background write alone deliberately.** `applyTheme()` writes `explorer-bg`/`explorer-bg-rgba` twice per *explicit theme change*, not per interaction, and the inline script in `src/app.html` reads both **synchronously at boot** to paint before first frame. Debouncing there would trade an invisible stall for a visible wrong-colour flash on the next launch.

## fix/506-git-repo-folder-icon (#506): flipping a default is a no-op for every existing install
- #471 diagnosed the missing git-repo tab icon correctly (the setting had defaulted off since birth) and fixed it by flipping `DEFAULT_SETTINGS.tabTitleGitRoot` to `true` — but that fix reached NEW installs only, so the icon was still missing and got re-reported as #506. Cause: `saveSettings` persists the WHOLE settings object (all 55 keys, to both localStorage and `settings.json`) and loading merges `{ ...DEFAULT_SETTINGS, ...saved }`, so a persisted key always beats its default. Every install that had ever saved settings kept the original `false` forever. Verified on the reporter's own machine: `~/.config/tauri-explorer/settings.json` still read `"tabTitleGitRoot": false`. **Whole-object persistence makes every default flip invisible to existing users — a bare `DEFAULT_SETTINGS` change is never a complete fix.** Ship a migration with it.
- The mock e2e suite is STRUCTURALLY unable to catch this class of bug, which is why `e2e/tab-title-git-root.spec.ts` was green through both issues. `mock-invoke`'s config store is an in-memory object that starts empty and `read_config_file` returns `""`, so localStorage is empty too and every mock run boots as a FRESH install — the only configuration in which the flipped default applies. Added `MOCK_CONFIG_SEED_KEY` so a test can pre-seed `settings.json` and present the app with an *upgraded* install. Any behaviour that depends on persisted state needs that seam, or the suite only ever tests day one.
- Fix shape: a `settingsVersion` stamp on the blob plus an append-only ledger in `src/lib/domain/settings-migration.ts`, each entry naming the keys whose persisted value is dropped in favour of the current default for installs stamped below it. It runs in `settingsStore.init()` and **only** there — `settings.json` is the durable store of record and localStorage is a cache `init` overwrites on every load, so migrating the cache separately would give the two stores different answers. Running once and writing straight back is what keeps it one-shot, which is what lets a user turn the setting off again afterwards.
- **Adding a version stamp to `DEFAULT_SETTINGS` silently arms every code path that writes the settings object.** `init()`'s fallback branch promotes the localStorage cache into `settings.json` when the config file is missing or unreadable; because the stamp now rides along in the defaults, that write marked a blob that had never been through the ledger as already migrated — disarming this and every future migration for that install, permanently. De-stamping just that one write is NOT enough and we shipped that mistake first: the **live** `settings` object still held the defaulted stamp, and `saveSettings` is the *other* writer of it, so a single `toggleSidebar()` re-stamped the file and disarmed the ledger again — measured, not theorised. The fix is to reset the stamp on the live object (`settings = { ...settings, [SETTINGS_VERSION_KEY]: 0 }`); `storedVersion` floors 0, so the next launch migrates normally while this launch's values are untouched. Reset it only when the cache is genuinely unstamped — zeroing unconditionally would re-run the whole ledger for a user who lost `settings.json` after migrating, reviving a decoration they had deliberately turned off. Whenever you add a "has been processed" marker to a defaulted object, audit **every writer of the object**, not every write site you happened to be editing — the marker's *absence* is the signal, defaults destroy absence, and de-stamping one serialization only hides the problem until the next save.
- That fallback is unreachable on an unmodified upgrade lifecycle (`settings.json` persistence landed 2026-03-11 in `b64299cb`, three months before `tabTitleGitRoot` existed at all in `22a0e320` 2026-06-24, and `saveSettings` writes both stores in one call — so no install can hold that key without also holding a config file). It is reachable when the config file is lost or unreadable while localStorage survives: a user deleting the config to "reset", a permission-blocked write, a warm-window boot reading mid-truncate. **"Unreachable on the happy path" is not "unreachable"** — and the one-line stamp reset costs nothing, whereas migrating the promoted blob outright would have flipped the live value and broken the existing `e2e/tab-title-git-root.spec.ts` fixture.
- **`writeConfigQueued` cannot be drained by a fixed number of macrotask turns.** It coalesces per filename and chains a queued write onto the in-flight one, so `await setTimeout(0)` — even a few of them — can read the file *between* two writes. A test that did exactly that passed for the wrong reason and only went red when the wait grew to 20 ms; the shipped form polls `readConfigFile` until the content is unchanged across several consecutive turns, which is a heuristic and is sound ONLY because every writer on this path is a contiguous promise chain with no timers — do not trust it against a debounced writer. Its saving grace is that it fails loudly (both call sites `JSON.parse` the drained content, so an early exit throws rather than passing for the wrong reason). If you need a real barrier, export one from `persisted.ts` that returns `inFlightWrites.get(filename)`; never `await writeConfigQueued(f, sameContent)`, which clobbers whatever is sitting in `pendingWrites`. If a settings test needs a sleep to be correct, it is asserting mid-flight state.
- An unstamped `tabTitleGitRoot: false` and a user's deliberate `false` are **logically indistinguishable** — that indistinguishability IS the bug. So the discriminator has to be the stamp, and the choice of *which* store to migrate decides which tests move. Migrating localStorage would have forced a fixture change in the existing `e2e/tab-title-git-root.spec.ts` (its hand-written 1-key blob carries no stamp and so reads as a legacy install); migrating only `settings.json` leaves that spec byte-identical, because it seeds localStorage and never populates the config file. When two tests want opposite outcomes from identical input, look for a layer where the inputs actually differ rather than editing the older test.

## fix/499 changed-files font: `var(--token, fallback)` fails silently when the token is never defined
- The git graph's changed-files rows styled themselves `font-family: var(--font-mono, monospace)`. **`--font-mono` is referenced 9× across the codebase and defined 0×** — there is no `:root { --font-mono }` anywhere — so every one of those declarations has always resolved to the browser's *generic* `monospace`, not to any intended stack. The changed-files list was the visible casualty (#499); the surviving sites (`.oid`, `.meta-mono`, `.ref-stash`, `.diff-lines`, the AI prompt input, `SettingsDialog`'s key display) still silently take the generic family. If you want a real mono stack there, define the token — the fallback is not a definition. Grep `var(--` against the `:root` block in `+page.svelte` before assuming a token exists.
- CSS custom properties have **no missing-token diagnostic**: no console warning, no devtools error, the rule just computes to the fallback. That is why this survived so long and why it needs a test rather than a review.
- Testing a CSS-only fix in this repo is awkward: vitest runs `environment: "node"` (no DOM, no cascade), and the changed-files typeface is only *truly* observable after the browser resolves the cascade. Two tiers, both asserting the observable and both verified to go red with the fix reverted: `e2e/git-graph-changed-files-font.spec.ts` reads `getComputedStyle(...).fontFamily` off the rendered rows (the real seam), and `tests/domain/git-graph-changed-files-font.test.ts` resolves the *actual* component declarations against the *actual* `:root` token table via new `domain/css-tokens.ts` helpers. Asserting on the CSS source text instead would be the wrong layer — the whole bug is that the source text looked fine and the resolved value didn't.
- Prefer `font-family: var(--font-family)` over `inherit` for a rule like this even when the parent already has `font: inherit`: it states the intent at the point of use and gives the unit tier something resolvable to assert on. (`.detail-file` does set `font: inherit`, so deleting the override would also work visually — but then no test can distinguish "correct" from "rule deleted entirely".)
- Watch the fixed-width status column: `.file-status` is `width: 14px`, so alignment of the file names never depended on a mono face — dropping mono there costs nothing. The inline diff is the opposite case and must stay mono; both specs carry an explicit guard asserting `.diff-lines` still resolves to a monospace family, so an over-broad "make the panel match the app font" edit fails.
- Naive CSS scanners must strip `/* … */` first. The explanatory comment added above the fix quotes a selector (`.detail-file { font: inherit }`), and those braces alone derailed the rule-matching regex — the test went red *after* the correct fix, for a reason that had nothing to do with the fix.

## fix/501-directional-pane-focus (#501): pane "focus" is a state id, and the split cluster's key tokens don't all survive Alt the same way
- "Move focus to the pane on the left" needs **no DOM focus work at all**. `ExplorerPane` registers its keydown listener on `window` and gates it with `$effect(() => { if (!isActive) return; ... })` where `isActive = windowTabsManager.activePaneId === paneId` — so the pane that answers arrow keys is decided by a string in state, not by `document.activeElement`. `setActivePane(id)` is the whole implementation; a `paneRef.focus()` would be redundant, and asserting on `document.activeElement` in a test would assert on the wrong thing. The user-visible seam is the `.explorer-pane.active` border (`showActiveBorder = dualPaneEnabled && isActive`), which is what the e2e spec asserts.
- Directional focus is **geometric, not tree-structural**. `leafInDirection` (domain/pane-layout.ts) reuses `leafRects` — until #501 that function had exactly one consumer (`dwindlePlacement`) — and requires a candidate to be beyond the source edge on the movement axis AND to overlap it on the perpendicular axis. Skipping the overlap test picks the corner-diagonal pane in a 2x2 grid (from top-left, "right" would tie between top-right and bottom-right); walking the split tree instead ("go up to the nearest row ancestor") picks the wrong pane whenever the neighbour column is itself split.
- `matchesShortcut` recovers the physical key from `event.code` only for `Key*`/`Digit*` codes, so under Alt a **letter** binding (`Alt+L`, `Alt+P`) matches layout-independently while a **punctuation** binding (`Alt+;`, `Alt+'`) still matches on `event.key`. That's fine on Linux/Windows and for Playwright (`Alt+Semicolon` / `Alt+Quote` deliver `;` / `'`), but macOS Option+`;`/`'` mangles `event.key`, so the punctuation half of this cluster is the part to re-verify on a real Mac. The pre-existing `Cmd+Alt+;`/`'` splits have the same exposure.
- Guard directional-focus commands on `dualPaneEnabled` (i.e. `countLeaves > 1`), not `activeTabIsExplorer` like the split commands. `findMatchingCommand`'s availability predicate skips `when`-failing commands, so a single-pane window never `preventDefault`s a bare `Alt+L`/`Alt+P` that some future feature may want.

## fix/500 long changed-file names: `word-break: break-all` is not a truncation strategy, and `flex-shrink` weights cannot express a priority
- The changed-files rows had `word-break: break-all` and nothing stopping them wrapping, so a long path reflowed **mid-word**: 2 lines at a 1280px window, 6 at 700px, inflating the `position:absolute` inline commit panel and pushing every graph row below it down. `break-all` prevents *overflow*, not *reflow* — it silently converts a horizontal problem into a vertical one, which is why nothing looked broken in a geometry sense (`scrollWidth === clientWidth` throughout) while the panel visibly ballooned. Every other column in the graph truncates; if you add a text column here, truncate.
- **Splitting a label into two differently-styled halves must not change its text.** `e2e/git-graph.spec.ts` and `git-graph-commit-panel.spec.ts` match these rows on their full path (`"src/index.css"`, `"src/file-16.ts"`). `splitPathForDisplay` (`domain/path.ts`) is therefore specified as a *partition* — `dir + name === path` for every input, no separator normalization, no trailing-slash stripping, unlike the neighbouring `basename`/`parentDir` — and the two spans are emitted **adjacent with no whitespace between the tags** in the `filePathLabel` snippet. A newline between them would inject a text node and break those specs; there is no formatter in this repo to reflow it, but write it on one line and say why.
- **`flex-shrink` is proportional, so lopsided weights cannot express "A yields before B".** The first fix gave `.file-dir` a 200× shrink factor over `.file-name`, reasoning the directory would absorb everything. It does not: at 1280px (column 588px, name 520px, dir 393px) the name still took its ~2px share and rendered an ellipsis while the directory had 70px left to give. Weights make the error small, never zero. The idiom that *does* encode a priority is `flex-shrink: 0` + `max-width: 100%` on the item that must survive — it yields nothing until it alone exceeds the container, then clamps and ellipsises.
- **That 2px bug was caught by reading the screenshot against its criterion, not by the test.** The AC-3 assertion (`scrollWidth > clientWidth`) was green at the 1600px viewport I had moved it to *because the deficit there was ~13px and the name's share rounded to nothing*. Moving a criterion's viewport to make it pass is the exact move that hides this class of bug — 1280px, where the name fits but the whole path does not, is the width that catches it. Pick the width where the constraint actually binds.
- Estimating text width from a wrapped multi-line layout is unreliable: the 69-char base name looked ~600px extrapolated from the 2-line block, measured 520px. Measure with `getBoundingClientRect()` before concluding a criterion is infeasible.

## feat/517 SCM sidebar fuzzy filter: a filter that hides rows must also disarm the things that hide rows
- The sidebar already had two independent row-hiding mechanisms before the filter: the store's dir-scoping (`filteredSummary`, #380) and per-repo collapsed tree folders. Stacking a text filter on top means a match can be *found* and still not *rendered* — a hit inside a collapsed folder looks exactly like the filter dropped it, with no affordance saying otherwise. `collapsedFolders` therefore resolves to an empty set while the query is non-empty, which also keeps `visibleTreePaths` (arrow-key order) honest without a second code path.
- "Zero rows" is now ambiguous, and the empty state is the only place that can disambiguate it: `stagedCount + … === 0` used to mean a clean working tree and now also means filtered-to-nothing. Shipping only the first message would have told users their repo was clean while six files sat behind a query. Any narrowing feature added to a list with an empty state needs the second message in the same change.
- **But "a query is set" is not the same as "the query is what emptied the list", and conflating them inverts the bug.** The first cut branched the empty state on `filterActive` alone and rendered the input on `dirPendingCount > 0`; those two conditions disagree exactly when a query outlives its rows — the pane walks into a subfolder with no changes, or the watcher reports a clean tree — and the panel then said "No files match" about a genuinely clean directory *and* unmounted the input, so the stale query could not even be seen, let alone cleared. Both rules now take the pre-filter pending count (`scmEmptyState`, `showScmFilterInput` in `domain/scm-filter.ts`), which is also what makes them unit-testable; the adversarial pre-PR pass found this by navigating, which no test in the first draft did.
- Blanking `collapsedFolders` for rendering leaves `toggleFolder` writing to the *real* per-repo set: a folder click while filtering looked like a no-op and then re-collapsed the folder once the query cleared. Whenever a mode overrides derived UI state, audit the writers of the underlying state too — `toggleFolder` early-returns while a filter is active.
- Filtering is a *view* concern and must not reach the commit path. `canCommit`/`commitButtonLabel` read `fullSummary` (repo-wide, pre-dir-scoping), so a query that hides every staged file leaves the Commit button enabled and committing exactly what it would have committed anyway — there is an e2e case pinning this, because the natural refactor (point everything at `summary`) silently turns a display filter into a destructive scoping rule.
- Reuse `domain/fuzzy-score.ts`'s `fuzzyScorePath` rather than `includes()`: it is what Quick Open uses, so `idx` → `index.css` behaves identically in both surfaces, and its 0-means-no-match contract makes the filter a one-line `score > 0` predicate. Sorting by score is stable in ES2019+, so equal-scoring rows keep git's ordering.

## fix/515 bulk-hide remote-only branches: a second filter axis, not a rewrite of the first
- The graph already had a *history*-level "Local branches only" (#381, `local_only` → `BranchType::Local` in the Rust revwalk) that drops everything remote. #515 is a different set: remote refs **with no local counterpart** — `origin/main` stays when local `main` exists, `origin/legacy-import` goes. Both toggles can be on at once; they are separate axes and neither subsumes the other.
- Implementing the toggle by *rewriting* `branchFilter` (unchecking the remote-only names) was the obvious move and the wrong one: it destroys the user's curated per-branch/author selection, and toggling it back cannot tell "was checked before" from "user unchecked it". It is its own persisted boolean, and the query is built from both axes at call time (`branchWalkQuery` in `domain/git-graph.ts`, which passes the selection through untouched and only derives the subtracted names — see the `exclude_branches` bullet below for why it must be a subtraction), mirroring how `localOnly` already behaves.
- **The branch list is loaded lazily by the popover, but the toggle needs it.** `branchList` is only fetched when the filter popover first opens, so a *persisted* toggle would silently do nothing on the first paint of a reopened graph — the one case where it matters most. `reload()` now refreshes the branch list itself while the toggle is on. The guard that keeps that safe lives in the domain function: an empty branch list returns the caller's selection unchanged, never `[]`, so a load failure can't be read as "every branch is remote-only" and empty the graph.
- Cache-key the toggle (`snapshotKey`, #416), not the derived branch array: the derived array depends on the lazily-loaded list, so keying on it lets a pre-load remount paint the *unfiltered* variant's rows under the filtered key.
- The author checkboxes read "checked when every branch this author created is shown", which made an author flip to unchecked the moment the bulk toggle hid one of their remote-only branches — while their local branch was still visibly selected. Branches the bulk toggle hides are excluded from `branchesByAuthor` entirely: out of play on one axis means out of play for the widgets that summarize that axis.
- The popover's e2e selectors address branch rows as `label.bf-row:not(.bf-local-only):not(.bf-all):not(.bf-author-row)` — an exclusion list that every new non-branch row has to be added to. The new option row is a `.bf-opt` (styled with `.bf-row`, classed separately) so existing specs keep passing untouched; prefer a distinct class over extending the `:not()` chain in every spec.
- `mock-invoke`'s `git_refs` had drifted from `MOCK_GRAPH_REFS`: the graph drew an `origin/legacy-import` chip that the branch filter's list did not contain, so no e2e could reach a remote-only branch through the popover despite a comment claiming the two mirror each other. When a mock has two views of the same fixture, a comment is not a constraint — the drift only surfaced because a new feature needed the second view.
- The popover is capped at `max-height: 320px`, which silently clips the bottom rows out of any screenshot of the full branch list. Narrowing it with its own search box (`origin`) is the cheap way to get all the rows a criterion is about into one legible frame.
- **The backend's seeding rule makes "all branches except X" un-expressible as a `branches` list.** `build_log` is an `if let Some(names) = &opts.branches { … } else { push_head(); every branch … }`, and `local_only` is documented as ignored whenever `branches` is set. Enumerating "everything except the remote-only refs" therefore silently (a) defeated an active "Local branches only" — a *hide* toggle made remote-ahead commits **appear** — and (b) dropped `push_head()`, so a detached HEAD's unreferenced commits vanished and an all-remote-only repo went blank. The fix is a subtractive `exclude_branches` applied to *both* seeding paths; the client sends the raw selection untouched. A pre-PR adversarial reviewer found this — the mock e2e could not, because `mock-invoke`'s `git_log` doesn't implement `local_only` at all, which is exactly the "green mock E2E is not evidence for backend semantics" trap in CLAUDE.md.
- Anything that changes which tips the walk is seeded from also invalidates the `#431` resume cursor (it is keyed to the unfiltered walk), so `useCursor` has to test the exclusion as well as the selection.

## feat/524 standing detached-HEAD badge: the flag has to come from the repo, not from a missing branch name
- `head_branch === null` is the obvious detached signal and it is wrong: it is *also* null on an unborn branch (fresh `git init`), where a red "DETACHED HEAD" banner would be pure noise. `GitLogPage` therefore carries its own `detached: bool` from `repo.head_detached()`, and `fetchPage0Snapshot`/`GraphSnapshot` pass it through rather than re-deriving it. Rust tests pin both directions (detached repo, unborn repo).
- The graph reloads on every mutating action, so putting the flag on the *log page* (already fetched per reload) costs nothing, while reading it from `git_refs` — the only command that had `detached` before — would have meant a second IPC call the graph otherwise only makes when the branch popover opens.
- A standing indicator has to be re-paintable from cache. The snapshot cache exists so a tab remount paints synchronously (#255); a flag that only arrives via a fresh fetch makes the badge blink out on every tab switch, which is exactly the "did I imagine that?" failure the issue is about. Both `cacheSnapshot` call sites (page-0 fetch *and* the infinite-scroll append) have to carry it.
- `mock-invoke` had no notion of HEAD attachment at all — `git_checkout` moved the HEAD ref and nothing else, so `head_branch` was recomputed as "first local branch decorating HEAD's commit" and a detached checkout onto a decorated commit would have reported that branch. The mock now models attachment (`mockCheckout(oid, branch)`): only a local branch name reattaches; an OID, a tag or a remote branch detaches, like git. Without that, the e2e could assert the badge only via a magic global — i.e. it would test the mock, not the flow.
- Put the banner *outside* `.graph-header`: that row's `padding-left` is set inline to clear the graph lanes and its children are resizable column headers, so an extra child there is a layout hazard. A sibling strip above it is also the honest structure — it belongs to the view, not to the columns.
