...
impl Manager {
    fn get_managed_objects(&self) -> Objects {
        ...
        HashMap::from([
            (
                OwnedObjectPath::try_from(DISK).unwrap(),
                HashMap::from([(
            ...
            (
                OwnedObjectPath::try_from(ID).unwrap(),
                HashMap::from([
                    (
                        ...
                        HashMap::from([
                            ...
                            (
                                "Drive".into(),
                                val(OwnedObjectPath::try_from(DISK).unwrap()),
                            ),
...
impl Filesystem {
    fn mount(&self, _options: HashMap<String, OwnedValue>) -> zbus::fdo::Result<String> {
        ...
        match self.state.load(Ordering::SeqCst) {
            3 => Err(zbus::fdo::Error::AccessDenied(
...
async fn native_discovery_mount_refresh_and_removal_use_the_desktop_service() {
    ...
    let discover =
        |mounts| enumerate_with_connection(mounts, sys.path(), labels.path(), Some(&client));
    ...
    assert_eq!(
        mount_with_connection(&client, ID).await.unwrap(),
        "/media/USB Backup"
    );
    ...
    assert_eq!(
        mounted.len(),
        1,
        "mount table and UDisks must merge to one volume"
    );
    ...
    let unmounted = discover("42 35 8:17 / /media/USB\\040Backup rw - ext4 /dev/sdb1 rw\n").await;
    assert_eq!(
        unmounted.len(),
        1,
        "unmount between snapshots must not duplicate the volume"
    );
    assert_eq!(unmounted[0].path, "", "newer desktop mount state wins");
    ...
    assert_eq!(
        discover("").await.len(),
        1,
        "insertion appears on the next refresh"
    );
}
#[tokio::test]
...
#[tokio::test]
async fn missing_service_keeps_mounted_discovery_and_returns_an_actionable_mount_error() {
    let (_bus, server, client, _, _) = bus().await;
    server
        .release_name("org.freedesktop.UDisks2")
        .await
        .unwrap();
    let sys = tempfile::tempdir().unwrap();
    let labels = tempfile::tempdir().unwrap();
    std::fs::create_dir_all(sys.path().join("sdb/sdb1")).unwrap();
    std::fs::write(sys.path().join("sdb/removable"), "1").unwrap();
    let drives = enumerate_with_connection(
        "42 35 8:17 / /mnt/USB\\040Backup rw - ext4 /dev/sdb1 rw\n",
        sys.path(),
        labels.path(),
        Some(&client),
    )
...
async fn encoded_unicode_and_malformed_alias_text_are_preserved_at_the_discovery_seam() {
    for (alias, expected) in [
        ...
        assert_eq!(drives[0].path, "/media/Backup");
    }
}
