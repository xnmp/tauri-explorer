...
- Implementing the toggle by *rewriting* `branchFilter` (unchecking the remote-only names) was the obvious move and the wrong one: it destroys the user's curated per-branch/author selection, and toggling it back cannot tell "was checked before" from "user unchecked it". Keeping it as its own persisted boolean and composing at query time (`effectiveBranchSelection` in `domain/git-graph.ts`) makes the composition total and unit-testable, mirroring how `localOnly` already behaves.
