...
- Git-graph fetch, pull, and remote-branch push share one client-ID network-operation lifecycle in `git-graph-refresh.ts`. Per ADR 0006, only the unbounded network phase is killable: pull checks cancellation after atomic fetch, then lets its local fast-forward finish and return undo; remote-delete cancellation reports that the remote may already have applied it (#528).
