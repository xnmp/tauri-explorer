...
│   │   │   ├── context-menu.svelte.ts  # Menu position/visibility
│   │   │   ├── dialogs.svelte.ts       # Modal dialog state
...
|-------|------|---------|-------------|
| `dialogStore` | `dialogs.svelte.ts` | Modal dialog state | No |
| `contextMenuStore` | `context-menu.svelte.ts` | Menu position/visibility | No |
| `clipboardStore` | `clipboard.svelte.ts` | Copy/cut/paste state | No |
| `undoStore` | `undo.svelte.ts` | Undo stack for file ops | No |
...
### Current Refactoring (architecture_consolidation.md)

| Issue | Current | Target |
|-------|---------|--------|
| `explorer.svelte.ts` | 448 lines (God object) | ~200 lines |
| `FileList.svelte` | 898 lines | ~500 lines |
| Svelte warnings | 9 | 0 |

### Planned Extractions

- `ColumnHeaders.svelte` from FileList
- `MarqueeSelection.svelte` from FileList
- `view-options.svelte.ts` from explorer
