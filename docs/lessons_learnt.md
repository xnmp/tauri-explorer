...
- **The context-menu zoom offset was a *double* division, not a wrong formula.** `contextMenuStore.open()` already called `adjustForZoom(x,y)` (÷zoom), and `ContextMenu.svelte`'s clamp effect divided the stored position by zoom *again* — so the menu landed at `clientX / zoom²` and drifted up-left as you zoomed in (invisible at 100%). Fix: store the raw `clientX/clientY` and let the component do the single division it already needs for viewport clamping. Lesson: when one coordinate transform looks correct in isolation, check whether an upstream store already applied it.
