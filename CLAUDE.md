...
| Native-suite TypeScript check          | `bun run check:e2e:tauri`                                                                                                                                                                                                                                                                                                                                                                                                          |
| Extended native soak (opt-in)         | From a clean worktree run `bun run build:native:qualification`, then `SOAK_DURATION_MS=14400000 SOAK_SEED=<repro-seed> SOAK_EXPECTED_DISPLAY_SCALE=<native-scale> bun run test:e2e:tauri:soak` (`SOAK_MAX_CYCLES=1` gives a bounded real-native check; the runner verifies the generated build manifest, and reports/screenshots land in `qualification-results/`)                                                               |
...
- Native qualification must bind resource sampling to the verified manifest binary, retain run-specific driver output on failures, and normalize Rust `Duration` units before calculating startup percentiles. macOS cold-start attribution correlates native monotonic readiness with browser wall-clock boundaries: retain the residual as `unattributedMs`, keep foreground and warm samples separate, and never describe two animation-frame callbacks as compositor-presented pixels or a Dock bounce. The pre-`run` interval is its own `processEntryMs` phase (main.rs hands `run_with_process_entry` its entry instant); time before `main()` is outside every in-process clock and comes only from the runbook's launch recording. Startup marks belong to one owner: `window-session` records `mount`/`settings-ready`/`commands-ready`, `+page.svelte` only `bundle-exec`/`list-ready`/`app-ready`. One parser serves both the readiness predicate and the report: an independent readiness regex drifted from the emitted log format and would have failed every Mac sample (#696). Reject a residual below -5ms and any duplicated webview marker rather than publish a plausible decomposition, and let a `missed` measured half-bounce deadline fail the run while `unqualified` stays non-fatal. Interactive Mac evidence is ingested only from inside the qualification root and must match the verified binary SHA-256, hardware model, normal Launch Services method, sample count, and per-sample recording/trace plus functional-frame/input times. Per ADR 0021, raw replay seeds never become path components; signal exits, rejected force-kills, and children still alive after the force timeout fail the run, with process output preserved. WebdriverIO does not turn ordinary `afterSession` rejection into a failing command, so owned-process cleanup errors cross the worker/launcher boundary through run-scoped state checked by `onComplete`.
- Per ADR 0022, the native E2E target cache is keyed by the exact Rust compiler identity as well as target inputs. Keep its restore and save actions separate: save only after the native contracts and embedded build succeed, but allow a later GUI failure to retain those safe build products. Contract/provenance diagnostics upload before GUI execution so a failed contract remains inspectable while independent native checks run (#694). GitHub's explicit Bash shell enables `-e -o pipefail`; disable errexit only around a piped diagnostic command, capture `PIPESTATUS[0]`, then restore errexit before writing its timing artifact and returning that exact status (#697).
- A fresh-window element command can block until WebKitWebDriver deletes the
  session. Diagnose that interval with bounded Node-side `/proc` samples only;
  do not issue another WebDriver command. Identify the newest selection-time
  `WebKitWebProcess` by PID plus start time (not PID alone), retain capture
  errors rather than treating them as process absence, and always clear the
  sampler without changing the lookup rejection (#703).
...
- Recycle Bin launch is platform-specific and belongs in `system::open_recycle_bin`: Windows uses `shell:RecycleBinFolder`, macOS opens `~/.Trash`, and Linux opens the absolute Freedesktop `Trash/files` directory through `xdg-open`.
- Never dispatch `trash:///` on Linux. `gio` can exit successfully after handing the URI to a desktop application that rejects it asynchronously, so its status cannot prove the Recycle Bin opened (#660, #672).
...
- Vertical preview docks retain the persisted preferred height, but their rendered wrapper caps it to the center stack's available height minus a usable file region. Keep that allocation in the shared page layout rather than component-local resize state so zoom and window resizing cannot feed back into saved settings (#699).
...
- Bookmark feedback must state the action for the source kind: directory drags always say `Drop to pin`, including a targeted bookmark row; only files on a targeted row say `Move to bookmark`. Resolve that kind from in-memory drag state or the persisted cross-window fallback, matching native drop handling (#675).
...
The preview pane requests a 1024px video frame because fullscreen reuses that
