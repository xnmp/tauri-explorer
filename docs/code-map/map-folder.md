...
- `git-graph-refresh.ts` — F5 refresh bus plus importable graph refresh machinery: `createReloader` serializes fetches with generation/dirty guards, `shouldReloadGraphForChange` filters local-action echoes, and `countGraphWalkCommits` excludes woven stash rows from numeric paging (#432, #444). It also owns the process-wide cancellable Git-operation state and consumes task phase events so pull stops offering Cancel during its protected local fast-forward (#528). GitGraphView registers its fetch+reload per pane id; the `gitGraph.refresh` command dispatches to the active pane.
...
- `git-network-operation.ts` — shared Tauri/DOM phase-event contract for crossing a Git network operation from cancellable network work into a protected local mutation (#528).
