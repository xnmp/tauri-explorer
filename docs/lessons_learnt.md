...
- Note the two callers: `TitleBar.svelte` derives `tabStripVisible` to decide whether the row renders, and `WindowTabBar.svelte` derives `showTabArea` to decide whether the strip renders. They duplicate the same `showWindowTabBar(...)` call, so any new argument must be threaded through **both** or the row and its contents disagree.
