...
describe("embedded terminal", () => {
  ...
  (process.platform !== "win32" ? it : it.skip)(
    ...
    async () => {
      // Keep this regression self-contained so it can run independently of
      // the shell lifecycle tests above.
      await $(".file-list").waitForExist({ timeout: 15_000 });
      await browser.keys(["Control", "`"]);
      await $(".terminal-panel .xterm").waitForDisplayed({ timeout: 10_000 });
      const input = $(".terminal-panel textarea.xterm-helper-textarea");
      ...
      await browser.waitUntil(
        async () => (await terminalText()).includes("key-probe-ready"),
        { timeout: 15_000, timeoutMsg: "terminal key probe never entered raw mode" },
      );
