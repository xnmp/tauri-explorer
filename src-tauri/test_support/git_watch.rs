...
fn lease_release_is_idempotent_across_recreated_repository_observation() {
    ...
    let f = fixture();
    let first = run(f.service.acquire(path.clone())).unwrap();
    let old = receive(&f.observers);
    run(f.service.release(first.id.clone())).unwrap();
    receive(&old.dropped);
    let second = run(f.service.acquire(path)).unwrap();
    let current = receive(&f.observers);
    assert_ne!(first.id, second.id);
    run(f.service.release(first.id)).unwrap();
    assert!(current.dropped.try_recv().is_err());
    run(f.service.release(second.id)).unwrap();
...
fn shared_references_recover_after_callback_error_without_remounting() {
    ...
    let f = fixture();
    let first = run(f.service.acquire(path.clone())).unwrap();
    let second = run(f.service.acquire(path.clone())).unwrap();
    ...
    fault(&failed);
    let third = run(f.service.acquire(path)).unwrap();
    assert!(f.observers.try_recv().is_err());
    run(f.service.release(first.id)).unwrap();
    run(f.service.release(second.id)).unwrap();
    assert!(replacement.dropped.try_recv().is_err());
    run(f.service.release(third.id)).unwrap();
...
fn failed_recovery_retries_and_final_release_cancels_future_attempts() {
    ...
    let path = dir.path().to_string_lossy().into_owned();
    let f = fixture();
    let lease = run(f.service.acquire(path.clone())).unwrap();
    let old = receive(&f.observers);
    ...
    run(f.service.release(lease.id)).unwrap();
    ...
    let other_lease = run(f
        .service
        .acquire(other.path().to_string_lossy().into_owned()))
    .unwrap();
    let other_observer = receive(&f.observers);
    run(f.service.release(other_lease.id)).unwrap();
...
fn failed_emission_is_retried_and_shutdown_drops_every_observer() {
    ...
    let path = dir.path().to_string_lossy().into_owned();
    let f = fixture();
    let lease = run(f.service.acquire(path)).unwrap();
    ...
    assert!(run(f.service.acquire(dir.path().to_string_lossy().into_owned())).is_err());
}
...
fn acquisition_rejects_a_callback_failure_during_registration() {
    ...
    .unwrap();
    assert!(run(service.acquire(dir.path().to_string_lossy().into_owned())).is_err());
    receive(&dropped);
}
...
fn canceled_acquisition_drains_the_late_native_observer() {
    ...
    run(async {
        ...
        let path = dir.path().to_string_lossy().into_owned();
        let pending = tokio::spawn(async move { owner.acquire(path).await });
...
fn native_observer_recovers_after_root_moves_and_observes_worktree_lock_files() {
    ...
    .unwrap();
    let path = dir.path().to_string_lossy().into_owned();
    let lease = run(service.acquire(path.clone())).unwrap();
    receive(&installed);
    ...
    let second = run(service.acquire(path)).unwrap();
    while changed.try_recv().is_ok() {}
    std::fs::write(dir.path().join("Cargo.lock"), "changed after recovery").unwrap();
    assert_eq!(receive(&changed), lease.repo_root);
    run(service.release(lease.id)).unwrap();
    run(service.release(second.id)).unwrap();
}
...
fn measure_retained_observation_fanout() {
    ...
    for samples in [&mut cold, &mut shared] {
        for dir in &repositories {
            let started = std::time::Instant::now();
            leases.push(run(service.acquire(dir.path().to_string_lossy().into_owned())).unwrap());
