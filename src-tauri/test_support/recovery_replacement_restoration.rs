...
fn every_endpoint_combination_has_a_conservative_next_step() {
    ...
    let allowed = [
        ((1, 0, 2), RestorationStep::ParkPublication),
        ((1, 0, 3), RestorationStep::ParkPublication),
        ((1, 0, 0), RestorationStep::RestoreOriginal),
        ((1, 1, 0), RestorationStep::RestoreOriginal),
        ((1, 2, 0), RestorationStep::RestoreOriginal),
        ((0, 0, 1), RestorationStep::Complete),
        ((0, 1, 1), RestorationStep::Complete),
        ((0, 2, 1), RestorationStep::Complete),
    ];
        ...
        for (p, private) in publications.into_iter().enumerate() {
            for (t, target) in targets.into_iter().enumerate() {
                let expected = allowed
                    ...
                    .map_or(RestorationStep::Conflict, |(_, step)| *step);
                assert_eq!(
                    restoration_step(retained, private, target, &original, &staged).unwrap(),
                    expected,
...
#[test]
fn changed_versions_and_aliases_do_not_grant_restoration() {
    let original = version(1, 0o100600);
    let staged = StagedPayload {
    ...
    changed.size += 1;
    assert_eq!(
        restoration_step(Some(&original), None, Some(&changed), &original, &staged).unwrap(),
        RestorationStep::Conflict
    );
    assert!(restoration_step(None, None, None, &staged.version, &staged).is_err());
    let malformed = StagedPayload {
        version: staged.version,
        final_mode: Some(0o700),
    };
    assert!(restoration_step(None, None, None, &original, &malformed).is_err());
}
