...
it("closing during Git watch acquisition releases the late native reference", async () => {
  ...
  api.watch.mockImplementation(() => new Promise((resolve) => { acquired = () => { refs++; resolve({ ok: true, data: { id: "lease", repoRoot: "/canonical-repo" } }); }; }));
  ...
  expect(refs).toBe(0);
  expect(api.unwatch).toHaveBeenCalledWith({ id: "lease", repoRoot: "/canonical-repo" });
});
