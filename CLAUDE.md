...
- Bookmarks have two drop surfaces: a specific `.bookmark-drop-target[data-path]` is a file-transfer destination, while empty `.quick-access` space remains the folder-pinning target. Resolve the row before its containing section so Linux native and macOS/Windows pointer drags agree (#675).
